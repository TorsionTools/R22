				...
				if(!string.IsNullOrWhiteSpace(txtSearch.Text))
				{
					...
					using(Transaction trans = new Transaction(doc))
					{
						...
						foreach(Element tBLock in tBlocks)
						{
							...
							{
								if(tBLock.get_Parameter(BuiltInParameter.SHEET_NUMBER).AsString().Contains(txtSearch.Text))
								{
							...
							{
								if(tBLock.get_Parameter(BuiltInParameter.SHEET_NAME).AsString().Contains(txtSearch.Text))
								{
									tBLock.LookupParameter(((Parameter)cboParameter.SelectedValue).Definition.Name).Set(Convert.ToInt32(rdoShow.Checked));
								}
							}
